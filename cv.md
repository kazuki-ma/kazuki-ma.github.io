...
この FDE では、外部戦略パートナーへの LINE Platform 導入支援や、導入支援を再利用可能な技術記事・サンプルコードとして外部化する活動も行いました。
...
* `2022-04 - 2025-01` / LINE Digital Frontier. Server-side senior software engineer and tech lead. - LINE Digital Frontier 株式会社。サーバーサイドシニアエンジニア、TechLead
* `2019-10 - 2022-03` / LINE Corporation. Server-side senior software engineer. - LINE 株式会社（現 LINEヤフー株式会社）。サーバーサイドシニアエンジニア
* `2018-10 - 2019-09` / [Cluster, Inc](https://corp.cluster.mu/). Server-side engineer of VR App and facility engineer etc.
* `2015-02 - 2018-09` / LINE Corporation. Server-side senior software engineer and Manager.
* `2013-01 - 2015-01` / JustSystems.
* `2012-04 - 2012-12` / Sony Global Solutions.
...
* HTML, CSS, JavaScript (+AltJS) - Server-side
* etc.
...
------------
* Senior Software Engineer. Backend engineer for Gen-AX's generative AI SaaS product.
* Public interviews / articles:
    * [「X-Boost（クロスブースト）」を支えるエンジニアたち ― LLM Opsと大規模インフラの裏側](https://note.com/gen_ax/n/n4f973da3cd55)
    * [最先端だけど等身大。生成AI SaaSに集まった“普通”のエンジニアたち](https://note.com/gen_ax/n/nf539a306a16d)
    * [生成AI特化型SaaSのプロダクト開発にRustを採用した理由と今後の技術選定](https://note.com/gen_ax/n/na864b956c1a1)
...
* [Kotlin Fest 2022](https://2022.kotlinfest.dev/) Speaker (based on CfP)
...
* [Kotlin Fest 2024](https://www.kotlinfest.dev/kotlin-fest-2024) Speaker (based on CfP; 採択率 17 / 111 = 15%)
...
* 標準的・基本的な Storage (MySQL, Redis, Elasticsearch) をなるべく理想的な形で運用し、安定的に 30,000 req/sec API を提供。
    ...
    * Elasticsearch を1級ストレージとして、MySQL-based listing からの移行を支援。
...
* ClickHouse 等を利用した、大規模データの分析基盤構築。（補完用途）
...
2019/10-2022/03 [LINE Corporation.](https://linecorp.com/)
------------
...
* General server-side engineer. (Golang)
    * Designed & implemented IAP (In-App Purchase) coin table & transaction
...
* Lead engineer of [LINE STORE](https://store.line.me).
...
* O2O (Online to Offline) Division, Forward Deployed Engineer (FDE) / TechLead (Manager).
    ...
    * 戦略パートナーとの連携を含む FDE として、LINE ショッピングでは Value Commerce / LinkShare、LINE トラベルでは [ベンチャーリパブリック社](https://markezine.jp/news/detail/28841) との連携に関わりました。
    * LINE トラベルでは、LINE Platform を外部パートナーのサービスに導入するため、パートナー側システム・社内 Platform・事業要件の前提をすり合わせ、API / ID 連携・運用・ローンチに関する技術調整を担当。
    * 事業部門と開発部門・協力会社の間で、要件整理、技術的な調整、サービス価値の共有を担当。
    * 複数のパートナー・事業で繰り返し発生する LINE Login / ID Federation 導入支援を、再利用可能なサンプルコードと技術記事として外部化しました。
        * Technical Blog Entry: [Spring Security + 設定ファイルで始める LINE との ID Federation](https://engineering.linecorp.com/ja/blog/spring-security-id-federation-line-starting-configuration-file)
...
* Full stack engineer & Product Owner of [GDMS](https://www.justsystems.com/jp/products/gdms/).
    ...
    * Java 6 -> Java 7 migration lead on Tomcat Servlet environment, JSP(HTML, CSS, JavaScript).
